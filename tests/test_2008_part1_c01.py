...
class TestABC_MRT16(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        audio_dir = os.path.join(os.curdir, '2008 study', 'audio2008_part1', 'c01_2008')
        
        #Specify talkers
        talkers = np.array(['F1', 'F3', 'M3', 'M4'])
       
        #For each condition, generate a list of audio file paths 
        cls.c01_file_paths = generate_file_paths(talkers, audio_dir, condition='c01')
        
        #Generate list of files numbers (will be used for all conditions)
        cls.file_num = [abcmrt.file2number(file) for file in cls.c01_file_paths]
         
        #For each condition, generate a list of speech vectors  
        cls.c01_speech = generate_speech_vectors(cls.c01_file_paths)

        #Generate file paths for csv files
        cls.python_csv = os.path.join('csv', 'python', '2008 part 1', 'python_2008_part1_c01.csv')
        cls.matlab_csv = os.path.join('csv', 'matlab', '2008 part 1', 'matlab_2008_part1_c01.csv')
        cls.diff_csv = os.path.join('csv','diff', '2008 part 1', 'diff_2008_part1_c01.csv')
        
    def test_impaired_c01(self): 
        c01_phi_hat, c01_success = abcmrt.process(self.c01_speech, self.file_num, verbose=True)
        
        generate_csv(c01_success, self.c01_file_paths, self.python_csv) 
