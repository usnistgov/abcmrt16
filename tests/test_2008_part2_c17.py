...
class TestABC_MRT16(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        audio_dir = os.path.join(os.curdir, '2008 study', 'audio2008_part2', 'c17_2008')
        
        #Specify talkers
        talkers = np.array(['F1', 'F3', 'M3', 'M4'])
       
        #For each condition, generate a list of audio file paths 
        cls.c17_file_paths = generate_file_paths(talkers, audio_dir, condition='c17')
        
        #Generate list of files numbers (will be used for all conditions)
        cls.file_num = [abcmrt.file2number(file) for file in cls.c17_file_paths]
         
        #For each condition, generate a list of speech vectors  
        cls.c17_speech = generate_speech_vectors(cls.c17_file_paths)

        #Generate file paths for csv files
        cls.python_csv = os.path.join('csv', 'python', '2008 part 2', 'python_2008_part2_c17.csv')
        cls.matlab_csv = os.path.join('csv', 'matlab', '2008 part 2', 'matlab_2008_part2_c17.csv')
        cls.diff_csv = os.path.join('csv','diff', '2008 part 2', 'diff_2008_part2_c17.csv')
        
    def test_impaired_c17(self): 
        c17_phi_hat, c17_success = abcmrt.process(self.c17_speech, self.file_num, verbose=True)
        
        generate_csv(c17_success, self.c17_file_paths, self.python_csv) 
