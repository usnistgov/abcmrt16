...
class TestABC_MRT16(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        audio_dir = os.path.join(os.curdir, '2008 study', 'audio2008_part2', 'c11_2008')
        
        #Specify talkers
        talkers = np.array(['F1', 'F3', 'M3', 'M4'])
       
        #For each condition, generate a list of audio file paths 
        cls.c11_file_paths = generate_file_paths(talkers, audio_dir, condition='c11')
        
        #Generate list of files numbers (will be used for all conditions)
        cls.file_num = [abcmrt.file2number(file) for file in cls.c11_file_paths]
         
        #For each condition, generate a list of speech vectors  
        cls.c11_speech = generate_speech_vectors(cls.c11_file_paths)

        #Generate file paths for csv files
        cls.python_csv = os.path.join('csv', 'python', '2008 part 2', 'python_2008_part2_c11.csv')
        cls.matlab_csv = os.path.join('csv', 'matlab', '2008 part 2', 'matlab_2008_part2_c11.csv')
        cls.diff_csv = os.path.join('csv','diff', '2008 part 2', 'diff_2008_part2_c11.csv')
        
    def test_impaired_c11(self): 
        c11_phi_hat, c11_success = abcmrt.process(self.c11_speech, self.file_num, verbose=True)
        
        generate_csv(c11_success, self.c11_file_paths, self.python_csv) 
