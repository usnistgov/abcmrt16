...
class TestABC_MRT16(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        audio_dir = os.path.join(os.curdir, '2008 study', 'audio2008_part2', 'c19_2008')
        
        #Specify talkers
        talkers = np.array(['F1', 'F3', 'M3', 'M4'])
       
        #For each condition, generate a list of audio file paths 
        cls.c19_file_paths = generate_file_paths(talkers, audio_dir, condition='c19')
        
        #Generate list of files numbers (will be used for all conditions)
        cls.file_num = [abcmrt.file2number(file) for file in cls.c19_file_paths]
         
        #For each condition, generate a list of speech vectors  
        cls.c19_speech = generate_speech_vectors(cls.c19_file_paths)

        #Generate file paths for csv files
        cls.python_csv = os.path.join('csv', 'python', '2008 part 2', 'python_2008_part2_c19.csv')
        cls.matlab_csv = os.path.join('csv', 'matlab', '2008 part 2', 'matlab_2008_part2_c19.csv')
        cls.diff_csv = os.path.join('csv','diff', '2008 part 2', 'diff_2008_part2_c19.csv')
        
    def test_impaired_c19(self): 
        c19_phi_hat, c19_success = abcmrt.process(self.c19_speech, self.file_num, verbose=True)
        
        generate_csv(c19_success, self.c19_file_paths, self.python_csv) 
